...
- **The live folder is also edited directly by other tools and sessions** (for example v1.12.x–1.13.0 on 21–23 Sep 2026: cell-voltage Float trigger, fresh-data checks, Bluetooth event log; imported into git as commit `3de09f3`). Never deploy blindly. Immediately before copying, compare every file you are about to overwrite with the last deployed commit (`git show <commit>:<file>`, ignoring line endings). If a live file differs, **stop**: import the live changes first (branch from the last deployed commit, commit the live files, merge into `main`, resolve conflicts, re-apply your work), and only then deploy. Copy only the files you changed, and never touch live runtime files.
- **Deploy only when the user asks for it in that message.** Building and testing a change in the source folder does not authorise putting it on the live server; ask.
